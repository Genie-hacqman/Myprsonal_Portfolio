...
const HomeProjectCard = ({ project }) => {
  ...
  return (
    <div className="mx-auto h-full w-full max-w-full perspective-[1000px]">
      <motion.div
      ...
      >
        <div
          className="absolute inset-0 flex flex-col overflow-visible rounded-[18px] bg-[#1b233d] p-[0.7rem] shadow-[0_6px_18px_rgba(100,100,111,0.2)]"
        ...
        >
          <div className="relative h-28 overflow-hidden rounded-[14px] bg-[linear-gradient(45deg,#049fbb_0%,#50f6ff_100%)]">
            ...
            <div className="absolute top-0 left-0 z-20 flex h-6.5 w-full items-center justify-between px-3">
              ...
              <div className="flex h-full items-center gap-1.5">
                <a href={project.github} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()} className="flex h-full items-center justify-center rounded-full bg-white/80 p-1.5 text-[#1b233d] transition hover:bg-white hover:text-white">
                  <FiGithub />
                </a>
                <a href={project.liveDemo} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()} className="flex h-full items-center justify-center rounded-full bg-white/80 p-1.5 text-[#1b233d] transition hover:bg-white hover:text-white">
                  <FiExternalLink />
                </a>
              </div>
            </div>
            <img src={project.image} alt={project.title} className="absolute inset-x-0 bottom-0 h-14 w-full object-cover opacity-90" />
          </div>
          ...
          <div className="mt-2.5 flex-1 rounded-[13px] bg-[#142039] p-[7px_8px_10px] text-center">
            ...
            <p
              className="mt-1.5 text-[12px] leading-5 text-slate-100"
        ...
        <div
          className="absolute inset-0 overflow-visible rounded-[18px] bg-[#10172a] p-[0.7rem] shadow-[0_6px_18px_rgba(100,100,111,0.2)]"
          style={{ backfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}
        >
          <div className="relative h-full rounded-[13px] border border-white/10 bg-[#131b2f] p-3.5 text-white">
            <span className="block text-[14px] font-bold uppercase tracking-[1px] text-cyan-300">{project.title}</span>
            <p
            ...
            <div className="mt-3 space-y-2 text-sm text-slate-100">
              <div className="rounded-xl border border-white/10 bg-slate-800/80 p-2">
              ...
              <div className="rounded-xl border border-white/10 bg-slate-800/80 p-2">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  <a href={project.github} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()} className="inline-flex items-center gap-2 rounded-full bg-slate-900 px-2 py-2 text-[11px] font-semibold text-white transition hover:bg-cyan-500">
                    <FiGithub /> GitHub
                  </a>
                  <a href={project.liveDemo} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()} className="inline-flex items-center gap-2 rounded-full bg-slate-900 px-2 py-2 text-[11px] font-semibold text-white transition hover:bg-cyan-500">
                    <FiExternalLink /> Demo
                  </a>
                </div>
              </div>
